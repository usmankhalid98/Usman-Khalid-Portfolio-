  ...
  <section id="home" className="pt-40 pb-20 px-6 relative overflow-hidden">
    ...
    <div className="max-w-4xl mx-auto text-center relative z-10">
      ...
      <div className="mt-16 flex flex-wrap justify-center gap-8 md:gap-12 text-slate-500 font-bold">
        <a href={`mailto:${PERSONAL_INFO.email}`} aria-label="Email Usman" className="hover:text-blue-600 transition-colors flex items-center gap-2">
          <Mail size={18} /> {PERSONAL_INFO.email}
        </a>
        <a href={`https://${PERSONAL_INFO.linkedin}`} target="_blank" rel="noopener noreferrer" aria-label="LinkedIn Profile" className="hover:text-blue-600 transition-colors flex items-center gap-1">
          <Linkedin size={18} /> LinkedIn
        </a>
        <a href={`https://github.com/${PERSONAL_INFO.github}`} target="_blank" rel="noopener noreferrer" aria-label="GitHub Profile" className="hover:text-blue-600 transition-colors flex items-center gap-1">
      ...
      {PROJECTS.map((proj, i) => (
        <div key={i} className="glass-card group p-8 rounded-[2.5rem] flex flex-col h-full hover:bg-slate-300/40 transition-all border border-slate-300 shadow-sm">
          <div className="mb-6 flex items-start justify-between">
            ...
            {proj.link && (
              <a href={proj.link} target="_blank" rel="noopener noreferrer" aria-label={`View ${proj.title} website`} className="text-slate-400 hover:text-blue-600 transition-colors">
          ...
          {INTERESTS.map((item, i) => (
            <div key={i} className="flex gap-4 p-5 rounded-3xl bg-slate-200/50 border border-slate-300 hover:bg-slate-200 transition-all shadow-sm">
              <div className="w-12 h-12 rounded-2xl bg-slate-300 flex items-center justify-center text-blue-700 flex-shrink-0 shadow-inner">
                {/* Fixed: Added missing Globe import from lucide-react */}
    ...
    <section id="lets-talk-strategy" className="py-32 px-6 max-w-4xl mx-auto text-center">
      <div className="mb-16">
        ...
        <div className="flex flex-col sm:flex-row flex-wrap justify-center gap-6 mb-16">
          <a href={`mailto:${PERSONAL_INFO.email}`} aria-label="Send Email" className="group glass-card px-10 py-5 rounded-2xl flex items-center justify-center gap-3 hover:bg-blue-600 hover:text-white transition-all text-blue-700 font-black border-slate-300 active:scale-95 min-w-[220px]">
            <Mail size={24} /> <span>Email Me</span>
          </a>
          <a href={`https://${PERSONAL_INFO.linkedin}`} target="_blank" rel="noopener noreferrer" aria-label="LinkedIn" className="group glass-card px-10 py-5 rounded-2xl flex items-center justify-center gap-3 hover:bg-blue-700 hover:text-white transition-all text-blue-700 font-black border-slate-300 active:scale-95 min-w-[220px]">
        ...
        {status === 'success' ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              <label htmlFor="name" className="text-xs font-black text-slate-500 uppercase ml-1">Full Name</label>
              <input required id="name" name="name" value={formData.name} onChange={handleChange} placeholder="Joe Bloggs" className="w-full bg-slate-300/30 border border-slate-300 rounded-2xl px-6 py-4 focus:ring-2 focus:ring-blue-500/20 focus:outline-none placeholder:text-slate-400 font-bold text-slate-800" />
            </div>

            <div className="space-y-2">
              <label htmlFor="email" className="text-xs font-black text-slate-500 uppercase ml-1">Email Address</label>
              <input required id="email" type="email" name="email" value={formData.email} onChange={handleChange} placeholder="joe@example.com" className="w-full bg-slate-300/30 border border-slate-300 rounded-2xl px-6 py-4 focus:ring-2 focus:ring-blue-500/20 focus:outline-none placeholder:text-slate-400 font-bold text-slate-800" />
            </div>

            <div className="space-y-2">
              <label htmlFor="message" className="text-xs font-black text-slate-500 uppercase ml-1">Message</label>
              <textarea required id="message" name="message" value={formData.message} onChange={handleChange} rows={4} placeholder="Brief your project objective..." className="w-full bg-slate-300/30 border border-slate-300 rounded-2xl px-6 py-4 focus:ring-2 focus:ring-blue-500/20 focus:outline-none placeholder:text-slate-400 resize-none font-bold text-slate-800"></textarea>
            </div>
...
const Footer = () => (
  <footer className="py-12 border-t border-slate-300/50 text-center text-slate-500 text-xs font-black uppercase tracking-widest bg-slate-100/30">
    <div className="max-w-7xl mx-auto px-6">
      <p>&copy; {new Date().getFullYear()} Strategic AI Advisory & Solutions Consulting | Usman Khalid</p>
    </div>
  </footer>
);
